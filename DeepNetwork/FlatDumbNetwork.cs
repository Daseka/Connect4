...
public class FlatDumbNetwork
{
    ...
    public FlatDumbNetwork(IReadOnlyList<int> structure)
    {
        ...
        _deltaValues = new double[totalNeuronCount];
        Biases = new double[totalNeuronCount - structure[0]];
        _previousBiases = new double[totalNeuronCount - structure[0]];
        _previousBiasesChange = new double[totalNeuronCount - structure[0]];
        _updateBiasesValues = new double[totalNeuronCount - structure[0]];
        Array.Fill(_updateBiasesValues, 0.001);
        ...
        _previousGradients = new double[totalWeightCount];
        _previousWeightChange = new double[totalWeightCount];
        _updateValues = new double[totalWeightCount];
        Array.Fill(_updateValues, 0.001);
    ...
    public double[] Calculate(double[] input)
    {
        ...
        int lastLayerStart = _values.Length - Structure[^1];
        return [.. _values.Skip(lastLayerStart).Take(Structure[^1])];
    }
    ...
    public void UpdateWeights()
    {
        for (int i = 0; i < Weights.Length; i++)
        {
            ...
            double prevGrad = _previousGradients[i];
            double step = _updateValues[i];
            double lastChange = _previousWeightChange[i];
            ...
            _updateValues[i] = step;
            _previousWeightChange[i] = weightChange;
        }

        for (int i = 0; i < Biases.Length; i++)
        {
            double grad = GradientBiases[i];
            double prevGrad = _previousBiases[i];
            double step = _updateBiasesValues[i];
            double lastChange = _previousBiasesChange[i];
            int sign = Math.Abs(grad * prevGrad) < NearNullValue ? 0 : Math.Sign(grad * prevGrad);
            ...
            _updateBiasesValues[i] = step;
            _previousBiasesChange[i] = biasChange;
        }
    }
    ...
    private (double[] gradients, double[] biases, double error) ComputeGradientsAndErrorNormal(double[][] trainingInputs, double[][] trainingOutputs)
    {
        ...
        for (int sample = 0; sample < trainingInputs.Length; sample++)
        {
            ...
            {
                double outVal = _values[lastLayerStart + node];
                double target = trainingOutputs[sample][node];
                _deltaValues[lastLayerStart + node] = (target - outVal) * ActivationFunctions[lastLayer].Derivative(outVal);
            ...
            for (int layer = 1; layer < numLayers; layer++)
            {
                ...
                {
                    GradientBiases[biasIndex] += _deltaValues[layerStart + node];
                    biasIndex++;
                }
            }
        }
    ...
    private (double[] gradients, double[] biases, double error) ComputeGradientsAndErrorSoftmax(double[][] trainingInputs, double[][] trainingOutputs)
    {
        ...
        for (int sample = 0; sample < trainingInputs.Length; sample++)
        {
            ...
            {
                outputValues[node] = _values[lastLayerStart + node];
            }
            ...
            {
                double target = trainingOutputs[sample][node];
                _deltaValues[lastLayerStart + node] = target - outputValues[node];
            ...
            for (int layer = 1; layer < numLayers; layer++)
            {
                ...
                for (int node = 0; node < currLayerSize; node++)
                {
                    for (int prevNode = 0; prevNode < prevLayerSize; prevNode++)
                    {
                        Gradients[weightIndex] += _deltaValues[currLayerStart + node] * _values[prevLayerStart + prevNode];
                        weightIndex++;
                    }
                }
            }
